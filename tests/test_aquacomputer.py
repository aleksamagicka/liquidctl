...
def test_octo_set_fixed_speeds_not_supported(mockOctoDevice):
    ...
    fan1_report, fan2_report = mockOctoDevice.device.sent

    assert fan1_report.number == 3
    assert fan1_report.data[0x59:0x59+3] == [0, 19, 136]  # 0, <5000>
    assert fan2_report.number == 3
    assert fan2_report.data[0xAE:0xAE+3] == [0, 32, 208]  # 0, <8400>
...
def test_quadro_set_fixed_speeds(mockQuadroDevice):
    mockQuadroDevice.set_fixed_speed("fan1", 50)
    mockQuadroDevice.set_fixed_speed("fan2", 84)

    fan1_report, fan2_report = mockQuadroDevice.device.sent

    assert fan1_report.number == 3
    assert fan1_report.data[0x35:0x35 + 3] == [0, 19, 136]  # 0, <5000>
    assert fan2_report.number == 3
    assert fan2_report.data[0x8A:0x8A + 3] == [0, 32, 208]  # 0, <8400>
